...
def _select_and_validate_ranking(
    ranking_path: Path, *, questions: dict[str, str], allowed_ids: set[str], modality: str,
    filters: dict[str, str], allow_zero_slots: bool,
) -> pd.DataFrame:
    ...
    for column, value in filters.items():
        if column not in ranking.columns:
            raise ValueError(f"ranking filter column is absent: {column}")
        ranking = ranking.loc[ranking[column].astype(str).eq(str(value))].copy()
    if ranking.empty:
        raise ValueError("ranking filters select no rows")
    ...
    ranking["zero_slot"] = ranking["item_id"].isna() | ranking["item_id"].astype(str).str.strip().eq("")
    if ranking["zero_slot"].any() and not allow_zero_slots:
        raise ValueError("ranking contains an unresolved item_id but zero slots are not authorized")
    ranking.loc[~ranking["zero_slot"], "item_id"] = ranking.loc[~ranking["zero_slot"], "item_id"].astype(str)
    ...
    for qid, group in ranking.groupby("qid", sort=False):
        ...
        candidate_ids = group.loc[~group["zero_slot"], "item_id"].tolist()
        if len(candidate_ids) != len(set(candidate_ids)):
            raise ValueError(f"{qid}: ranking contains duplicate candidates")
    outside = set(ranking.loc[~ranking["zero_slot"], "item_id"]) - allowed_ids
    if outside:
...
def freeze_jobs(
    *, rankings_path: Path, questions_path: Path, texts_path: Path, candidate_order_path: Path,
    ...
    filters: dict[str, str] | None = None, allow_zero_slots: bool = False,
) -> dict[str, Any]:
    ...
    if set(filters) & {"qid", "rank", "item_id", "modality"}:
        raise ValueError("ranking filters cannot alter the E030 coverage or ranking contract")
    ...
    ranking = _select_and_validate_ranking(
        rankings_path, questions=questions, allowed_ids=allowed_ids, modality=modality, filters=filters,
        allow_zero_slots=allow_zero_slots,
    )
    source_hashes = {
    ...
    for row in ranking.to_dict("records"):
        zero_slot = bool(row["zero_slot"])
        candidate_id = None if zero_slot else str(row["item_id"])
        card = None if zero_slot else cards.get(candidate_id)
        if not zero_slot and card is None:
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--allow-zero-slots", action="store_true")
...
def parse_filters(raw_filters: list[str]) -> dict[str, str]:
    ...
    for raw_filter in raw_filters:
        ...
        if not separator or not column or not value or column in filters:
            raise ValueError("each --filter must be one unique non-empty COLUMN=VALUE pair")
...
def main(argv: list[str] | None = None) -> int:
    ...
    receipt = freeze_jobs(
        ...
        filters=parse_filters(args.filter), allow_zero_slots=args.allow_zero_slots,
    )
