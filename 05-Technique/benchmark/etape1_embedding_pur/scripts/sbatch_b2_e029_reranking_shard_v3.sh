#!/usr/bin/env bash
# Execute one immutable E029 output-budget successor shard.
# The historical v2 launcher is deliberately left unchanged.
#SBATCH --job-name=lkg-b2-e029-o512
#SBATCH --gres=gpu:1
...
if payload.get("experiment_id") != "E029" or payload.get("status") != "frozen_authorized_model_execution_v2":
    raise SystemExit("unexpected E029 successor execution manifest")
...
if payload["model"].get("temperature") != 0 or int(payload["model"].get("max_output_tokens", 0)) != 512:
    raise SystemExit("unexpected E029 successor generation contract")
...
for name, value in {
    ...
    "MAX_OUTPUT_TOKENS": payload["model"]["max_output_tokens"],
...
if [[ -f "$MATERIALIZED/materialization_receipt.json" ]]; then
  echo "E029 successor shard $E029_SHARD is already materialized; immutable output preserved."
  exit 0
fi
...
"$PYTHON_BIN" - "$RUN_ROOT" "$E029_SHARD" "$actual_manifest_sha" "$RUNNER" "$JOBS" "$MAX_OUTPUT_TOKENS" <<'PY'
import hashlib
import json
import os
import subprocess
...
payload = {
    ...
    "max_output_tokens": int(sys.argv[6]),
