...
runner=p['code_bundle']['v3_fail_closed_runner']; context=s['context_audit']
for k,v in {'MODEL':m['id'],'MODEL_REVISION':m['revision'],'SNAPSHOT':m['snapshot'],'PORT':s['vllm_port'],'OUT':p['outputs']['root'],'JOBS':s['jobs']['path'],'JOBS_SHA':s['jobs']['sha256'],'CONTEXT':context['path'],'CONTEXT_SHA':context['sha256'],'PROMPT':prompt['path'],'PROMPT_SHA':prompt['sha256'],'RUNNER_SHA':runner['sha256'],'MAXLEN':m['max_model_tokens'],'MAXTOK':m['max_output_tokens'],'WORKERS':m['max_workers']}.items(): print(f'{k}={shlex.quote(str(v))}')
PY
)"; eval "$assignments"
RUN_ROOT="$LKG_DATA_ROOT/$OUT/shards/$E030_SHARD"; mkdir -p "$RUN_ROOT"; JOBS="$LKG_DATA_ROOT/$JOBS"; CONTEXT="$LKG_DATA_ROOT/$CONTEXT"; PROMPT="$LKG_REPO/$PROMPT"
[[ -f "$JOBS" && -f "$CONTEXT" && -f "$PROMPT" && -x "$PYTHON_BIN" && -x "$VLLM_BIN" ]] || exit 2
[[ "$(sha256sum "$JOBS"|awk '{print $1}')" == "$JOBS_SHA" && "$(sha256sum "$CONTEXT"|awk '{print $1}')" == "$CONTEXT_SHA" && "$(sha256sum "$PROMPT"|awk '{print $1}')" == "$PROMPT_SHA" && "$(sha256sum "$RUNNER"|awk '{print $1}')" == "$RUNNER_SHA" ]] || exit 2
[[ "$(basename "$(readlink -f "$SNAPSHOT")")" == "$MODEL_REVISION" ]] || exit 2
...
PY
GPU="$(nvidia-smi --query-gpu=name,memory.total --format=csv,noheader | tr '\n' ';' | sed 's/;$//')"
printf '{"status":"complete_unaggregated","execution_manifest_sha256":"%s","jobs_sha256":"%s","context_audit_sha256":"%s","prompt_sha256":"%s","runner_sha256":"%s","node":"%s","slurm_job_id":"%s","port":%s,"listener":"127.0.0.1","gpu":"%s","model_id":"%s","model_revision":"%s","served_model_id":"%s"}\n' "$E030_EXECUTION_MANIFEST_SHA" "$JOBS_SHA" "$CONTEXT_SHA" "$PROMPT_SHA" "$RUNNER_SHA" "${HOSTNAME:-unknown}" "${SLURM_JOB_ID:-manual}" "$PORT" "$GPU" "$MODEL" "$MODEL_REVISION" "$SERVICE_MODEL" > "$RUN_ROOT/run_receipt.json"
