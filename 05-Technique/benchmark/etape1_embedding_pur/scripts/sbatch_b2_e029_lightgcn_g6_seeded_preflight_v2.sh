#!/usr/bin/env bash
#SBATCH --job-name=lkg-b2-e029-g6-seeded-preflight
...
# E029 preparation only.  It creates a successor set of compatible G6 pools
# with replay_seed in every job payload, then audits exact chat-template tokens.
# It never starts vLLM and never calls a reranker.
set -eEuo pipefail

: "${LKG_REPO:?Set LKG_REPO to the reproducibility checkout.}"
: "${LKG_DATA_ROOT:?Set LKG_DATA_ROOT to the data checkout.}"
: "${E029_SEEDED_PREFLIGHT_MANIFEST_SHA:?Pass the SHA-256 of the frozen manifest.}"

PYTHON_BIN="${LKG_PYTHON:-$HOME/work/.venv-benchmark/bin/python}"
ROOT="$LKG_REPO/05-Technique/benchmark/etape1_embedding_pur"
MANIFEST="$ROOT/configs/b2_reranking_comparable_a3_lightgcn_g6_seeded_preflight_v2.json"
...
[[ "$actual_manifest_sha" == "$E029_SEEDED_PREFLIGHT_MANIFEST_SHA" ]] || {
  echo "seeded E029 preflight manifest SHA-256 mismatch" >&2
  exit 2
}
...
if payload["experiment_id"] != "E029" or payload["status"] != "authorized_seeded_cpu_preflight_no_model_call":
    raise SystemExit("unexpected seeded E029 preflight manifest identity or status")
...
a3 = payload["a3"]
parent = payload["parent_preflight"]
...
emit("PARENT_MANIFEST_REL", parent["manifest_path"])
emit("PARENT_MANIFEST_SHA", parent["manifest_sha256"])
emit("PARENT_RECEIPT_REL", parent["receipt_path"])
emit("PARENT_RECEIPT_SHA", parent["receipt_sha256"])
...
emit("ARTICLE_K_INS", " ".join(map(str, payload["reranking_contract"]["compatible_conditions_per_seed"]["article"])))
emit("JP_K_INS", " ".join(map(str, payload["reranking_contract"]["compatible_conditions_per_seed"]["jp"])))
...
PARENT_MANIFEST="$LKG_REPO/$PARENT_MANIFEST_REL"
PARENT_RECEIPT="$LKG_DATA_ROOT/$PARENT_RECEIPT_REL"
...
for path in "$A3" "$QUESTIONS" "$PARENT_MANIFEST" "$PARENT_RECEIPT" "$RANKING" "$ARTICLE_TEXTS" "$JP_TEXTS" "$ARTICLE_PROMPT" "$JP_PROMPT" "$MODEL_SNAPSHOT"; do
...
[[ "$(sha256sum "$PARENT_MANIFEST" | awk '{print $1}')" == "$PARENT_MANIFEST_SHA" ]] || { echo "parent manifest SHA mismatch" >&2; exit 2; }
[[ "$(sha256sum "$PARENT_RECEIPT" | awk '{print $1}')" == "$PARENT_RECEIPT_SHA" ]] || { echo "parent receipt SHA mismatch" >&2; exit 2; }
...
[[ ! -e "$OUT_ROOT/preflight_receipt.json" ]] || {
  echo "seeded E029 preflight receipt already exists; leaving immutable output untouched."
  exit 0
}
...
for replay_seed in $REPLAY_SEEDS; do
  for modality in article jp; do
    ...
    for k_in in $k_ins; do
      ...
      [[ ! -e "$pool" ]] || { echo "immutable pool already exists: $pool" >&2; exit 1; }
  ...
  audit_args=()
  for k_in in $ARTICLE_K_INS; do audit_args+=(--jobs "$OUT_ROOT/pools/lightgcn_article_seed${replay_seed}_k${k_in}.jsonl"); done
  for k_in in $JP_K_INS; do audit_args+=(--jobs "$OUT_ROOT/pools/lightgcn_jp_seed${replay_seed}_k${k_in}.jsonl"); done
...
receipts = []
for seed in seeds:
    audit = root / f"fulltext_context_audit_lightgcn_g6_seed{seed}.json"
    payload = json.loads(audit.read_text(encoding="utf-8"))
    conditions = payload.get("conditions", [])
    if len(conditions) != 9 or any(row.get("questions") != 754 or not row.get("compatible") for row in conditions):
        raise SystemExit(f"invalid compatible audit for seed {seed}")
    pools = sorted(root.glob(f"pools/lightgcn_*_seed{seed}_k*.jsonl"))
    if len(pools) != 9:
        raise SystemExit(f"invalid seeded pool count for seed {seed}: {len(pools)}")
    receipts.append({
        "seed": int(seed),
        "audit": {"path": str(audit), "sha256": hashlib.sha256(audit.read_bytes()).hexdigest(), "conditions": conditions},
        "pools": [{"path": str(path), "sha256": hashlib.sha256(path.read_bytes()).hexdigest()} for path in pools],
    })
receipt = root / "preflight_receipt.json"
receipt.write_text(json.dumps({
    "experiment_id": "E029",
    "kind": "LightGCN_G6_seeded_fulltext_context_preflight",
