...
- E030 v1 (`986652`--`986673`) a échoué avant tout appel modèle car Slurm l'a placée sur P100 (capacité CUDA 6.0, inférieure au minimum 7.0 exigé par Gemma compressed-tensors). Cette archive ne produit aucun score. Le successeur E030 v2, manifeste `configs/b2_llm_as_judge_a3_execution_v2_gpu70.json`, SHA `7eb5d08758b3f833cfdb0f2c880066b8cbbe481cd2c252f20ed5582c8e3ea16f`, réutilise les 22 listes 754 × 10 inchangées mais limite Slurm à A100/L40S/H100 et refuse toute capacité GPU <7.0 au runtime. Smoke sans complétion `987034` vert sur A100 40 Go (capacité 8.0); shards `987039`--`987060` soumis. Aucun score de juge n'est encore disponible.
...
- Les cinq lots L40S ont été soumis : cosine `986445`, PPR `986446`, LightGCN graines `42/43/44` = `986447/986448/986449`; l'agrégation CPU `986450` dépend de leur succès intégral. Cosine a terminé `0:0` (4 h 29 min 15 s), avec 10 556 réponses pour 10 556 jobs et ranking top-10 SHA `654b191a9a45b25e04581c2ebee75d69a5261a5c905cc9fe68e45b08a5256087`; son reçu de matérialisation vaut `f85fa3e676bc756b3dda428f0c407ffd611e3042fe1c3e813270cf8952380659`. Les quatre autres shards sont encore actifs. Ne pas reprendre de métrique E029 tant que l'agrégation hashée ne confirme pas la matrice entière.
- Le préflight E030 non-reranké est terminé : `986458` (`0:0`, 4 min 35 s), reçu `.../_campaign_b2_e030_a3_nonreranked_ppr_light_context_preflight_v1_20260909/preflight_receipt.json`, SHA `c7b8c2a7607edd49f0ae08ab06d9ff4115aa5e735a910a4d69b26a90eae73138`. Il gèle et audite les huit sources PPR/LightGCN (754 × 10 positions chacune), avec zéro slot vide, zéro dépassement et zéro appel modèle. Avec cosine et le LLM direct déjà prêts, E030 n'attend plus que les listes finales E029.
...
- Le gate de budget est maintenant prêt : `scripts/114_audit_b2_e030_context.py`, SHA `f3346886e0e8b2a8b76dcd88f007356e15c98b9e7a1b65095aa35e39239e7aa1`, calcule les tokens du vrai message chat du tokenizer local et la réserve de sortie avant tout appel. Il émet un reçu hashé sans contenu de fiche ; toute incompatibilité bloque la soumission plutôt que de transformer silencieusement un échec technique en jugement. Trois tests dédiés et un smoke-test Télécom (25 tokens d'entrée + 256 réservés, zéro appel) passent. Aucun score E030 n'est encore produit.
- La soumission reproductible est aussi prête : `scripts/sbatch_b2_e030_judge_v1.sh`, SHA `dad8f4505cf2136c4c08be658de16cf76811726be586cb129f18edc9958dba96`, vérifie le manifeste E030 à créer, chaque hash de lot/prompt/audit et refuse un audit incompatible avant de démarrer vLLM. Il traite un shard par GPU et matérialise un reçu complet ; il ne peut pas choisir de retrieval ou modifier les listes. Son test de chemins obligatoires et `bash -n` passent. Aucun job E030 n'est soumis.
- Les listes top-10 déjà disponibles sont préparées de façon déterministe, sans modèle : `scripts/115_project_frozen_topk.py`, SHA `e746c7e9812ee8d29c41184d1fb27616d35afb252d45607b7a80ea2470014ce2`, a produit les projections A3 cosine (15 080 lignes, reçu `1c067e9e3e2edb5362ba49bd7814488453875a388dfa8d45994a172fa8543584), PPR (15 080, reçu `c5c03c681eb30bda3205742972861e716f45b890a14dc4de65f89721bad81066) et LightGCN G6 (45 240, graines séparées, reçu `bf7515c30fde173a651c0588b9fe874b458e9a985f35ca29ff22421e08d54cae). Elles n’apportent aucun score et seront jointes aux listes E029 K=70 seulement après l’agrégation contrôlée.
...
### 2026-09-18 — Baseline agentique web / MCP OpenLegy disponible

- Décision ou résultat : une campagne complémentaire d'agents juridiques est intégrée sous `harness-agent-legal/`. Le même agent `moonshotai/kimi-k2.6` (OpenRouter, température 0) reçoit une question pénale et renvoie jusqu'à dix références ordonnées. La condition `web` utilise Brave ; `openlegy_mcp` ajoute les outils MCP OpenLegy / Légifrance.
- Chiffres à reprendre, uniquement comme baseline agentique interne : web = Articles `NHit@10 0,2727`, `NDCG@10 0,2397`, `MRR@10 0,2564`; JP `0,0821`, `0,0769`, `0,0735`. OpenLegy MCP = Articles `0,3264`, `0,2673`, `0,2875`; JP `0,1456`, `0,1248`, `0,1198`.
- Sources : `harness-agent-legal/results_public/agentic_baseline_pool2runs.csv`, SHA-256 `4c48156b98b89617b0efc5f768d40cd0a308d1244d3ef613f892c8fb9f56eefd`; manifeste de provenance `harness-agent-legal/results_public/manifest.json`, SHA-256 `f29ed197dacb3058886165464720ee2a69f6929c491a33f177506b47cdc601e6`; description et commandes `harness-agent-legal/README.md`.
- Formulation autorisée : « Nous avons également évalué un agent Kimi K2.6 avec accès soit au Web, soit à OpenLegy via MCP, sur les mêmes deux tâches de récupération. OpenLegy améliore les scores observés par rapport à la configuration Web dans cette campagne interne. »
- Réserve à conserver dans méthode/limites ou légende : l'agrégat prend le meilleur NHit observé par cellule parmi deux exécutions historiques et deux QID n'ont pas été matérialisés à cause d'une limite de nom de fichier. Il porte donc le statut `exploratoire` et constitue une baseline agentique séparée, non une nouvelle ligne de comparaison confirmatoire A3. Ne pas en déduire un effet causal général du MCP ni une supériorité sur PPR/LightGCN.
- Action demandée : le Papier peut ajouter un tableau ou un paragraphe complémentaire « Agentic baselines » en séparant Articles et JP, sans identifiant E045 dans le texte publié. Ne pas ajouter de question, vérité terrain, trace d'outil ou CSV par question au manuscrit ou à GitHub.
