---
tags:
  - article
  - graphrag
categorie: GraphRAG
titre_complet: "Graph Retrieval-Augmented Generation: A Survey (ACM TOIS)"
auteurs: Peng et al.
annee: 2024
type: Survey
venue: ""
url: https://dl.acm.org/doi/pdf/10.1145/3777378
doi: ""
pdf_local: ""
status: a-lire
pertinence: haute
created: 2026-04-09
modified: 2026-04-09
---

# Graph Retrieval-Augmented Generation: A Survey (ACM TOIS)

> [!info] Metadonnees
> **Auteurs** : Peng et al.
> **Annee** : 2024 | **Venue** : Survey
> **Type** : Survey
> **URL** : [Lien](arxiv.org/abs/2408.08921)

## Resume

Premiere survey systematique de GraphRAG. Formalise le workflow : G-Indexing, G-Retrieval, G-Generation. Applications, benchmarks, cas industriels.

## Contributions principales

1. 
2. 
3. 

## Methodologie

### Donnees
- Source : 
- Volume : 
- Langue : 

### Architecture / Pipeline

*A completer apres lecture.*

## Resultats cles

| Metrique | Valeur | Baseline |
|---|---|---|
| | | |

## Points forts

- 

## Limites

- 

## Liens avec mon projet

> [!important] Pertinence pour le KG juridique francais
> *A completer apres lecture.*

### Ce que je peux reutiliser
- 

### Ce que je dois adapter
- 

## Connexions

### Articles lies
- [[]]

### Concepts lies
- [[]]

### Questions soulevees
- [[]]

## Citations cles

> "..." (p. X)

## Notes personnelles

*A completer apres lecture.*
