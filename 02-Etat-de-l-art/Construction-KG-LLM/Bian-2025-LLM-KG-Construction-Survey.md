---
tags:
  - article
  - construction-kg-llm
categorie: Construction-KG-LLM
titre_complet: "LLM-empowered Knowledge Graph Construction: A Survey"
auteurs: Bian et al.
annee: 2025
type: Survey (arXiv)
venue: ""
url: https://arxiv.org/pdf/2510.20345
doi: ""
pdf_local: ""
status: a-lire
pertinence: haute
created: 2026-04-09
modified: 2026-04-09
---

# LLM-empowered Knowledge Graph Construction: A Survey

> [!info] Metadonnees
> **Auteurs** : Bian et al.
> **Annee** : 2025 | **Venue** : Survey (arXiv)
> **Type** : Survey (arXiv)
> **URL** : [Lien](arxiv.org/abs/2510.20345)

## Resume

Survey complete : ontology engineering, extraction, fusion via LLM. Paradigmes schema-based vs schema-free. Etat de l'art des pipelines.

## Contributions principales

1. 
2. 
3. 

## Methodologie

### Donnees
- Source : 
- Volume : 
- Langue : 

### Architecture / Pipeline

*A completer apres lecture.*

## Resultats cles

| Metrique | Valeur | Baseline |
|---|---|---|
| | | |

## Points forts

- 

## Limites

- 

## Liens avec mon projet

> [!important] Pertinence pour le KG juridique francais
> *A completer apres lecture.*

### Ce que je peux reutiliser
- 

### Ce que je dois adapter
- 

## Connexions

### Articles lies
- [[]]

### Concepts lies
- [[]]

### Questions soulevees
- [[]]

## Citations cles

> "..." (p. X)

## Notes personnelles

*A completer apres lecture.*
